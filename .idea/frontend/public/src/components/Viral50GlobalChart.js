...
export const options = {
  plugins: {
    title: {
      display: true,
      text: "Popularność artystów w kategorii Viral50 - cały świat",
      font: {
        size: 15,
        weight: "bold",
      },
...
export const Viral50GlobalChart = () => {
  const [content, setContent] = useState([]);
  const [page, setPage] = useState(1);
  const [count, setCount] = useState(0);
  const xLabels = [];
  const yLabels = [];
  const apiURL = `http://localhost:8080/api/v1/viral50/Global?page=${page}&sortDate=DESC&sortRank=ASC`;
...
export default Viral50GlobalChart;
