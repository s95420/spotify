...
export const options = {
  plugins: {
    title: {
      display: true,
      text: "Popularność artystów w kategorii Viral50 - Polska",
      font: {
        size: 15,
        weight: "bold",
      },
...
export const Viral50PolandChart = () => {
  const [content, setContent] = useState([]);
  const [page, setPage] = useState(1);
  const [count, setCount] = useState(0);
  const xLabels = [];
  const yLabels = [];
  const apiURL = `http://localhost:8080/api/v1/viral50/Poland?page=${page}&sortDate=DESC&sortRank=ASC`;
...
export default Viral50PolandChart;
